...
def rechercher(q: str, limite: int = 8, niveau: str = None, matiere: str = None) -> list[dict]:
    """
    Recherche multi-mots INDEPENDANTE DE L'ORDRE. "maths bac c" et
    "bac c maths" donnent exactement les memes resultats -- chaque
    mot de la requete doit apparaitre quelque part dans le libelle,
    peu importe la position (contrairement a un LIKE '%phrase%'
    classique qui exige l'ordre exact).

    Deux niveaux de priorite fusionnes :
    1. La phrase complete apparait telle quelle (signal le plus fort)
    2. Tous les mots sont presents, ordre quelconque (signal correct)

    Remonte aussi niveau/matiere/serie de chaque ligne -- necessaires
    a calculer_score() pour les bonus par serie.
    """
    conn = get_connection()
    try:
